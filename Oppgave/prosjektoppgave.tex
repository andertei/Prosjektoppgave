...
The results presented in figure \ref{fig:arcingVoltage400A} and \ref{fig:arcingVoltage630A} illustrates the arcing voltage during failed interruptions tests inside the nozzle at 400 A and 630 A, at different upstream pressures. Since each line represents one test uncertainties are connected to these results, but general trends may be pointed out.

For each experiment the light blue line represents the lowest pressure tested and the orange line represent the highest, respectably 0.6 bar and 2.0 bar upstream pressure. When regarding pressure the same trend can be seen for both current levels, although some variations can be observed from pressure level to pressure level. If the pressure difference of 0.6 bar is compared to 2.0 bar some properties of the arcing voltage can be seen. 

For high current magnitudes the arcing voltage is approximately the same and constant even though the current is changing. But when the current approaches CZ the arcing voltage starts to rise. The difference between the to pressure levels can be observed in this area. For the low pressure test the arcing voltage starts to rise at the same time as the high pressure voltage, but the amplitudes of the arcing voltage is different. For the low pressure test the arcing voltage have a smaller amplitude than the high pressure test. This is probably due to a change in the efficiency of the cooling in the arc.

As described in section \ref{sec:HeatTransport} in figure \ref{fig:tempDist1} it is hard to cool the arc core with an air flow, since the temperature in the core is mainly dependent on the current. However as the current approaches zero the temperature in the core will decrease, as well as the electrical conductivity of the core which is indicated in figure \ref{fig:condAir}. The air flow influence the temperature in the arc with cooling of region two and three in figure \ref{fig:tempDist1}. These regions contains energy in the form of heat, and acts as an energy bank for the core of the arc. While blowing with a low pressure more energy is stored in these regions and the cooling of the arc core uses longer time. If little energy is stored in these regions, as for the high pressure test, a fast transaction from high to low temperature occurs in the arc. This is probably the reason for the steeper raise and higher amplitude of the arcing voltage for the high pressure test.

When regarding the impact on arcing voltage form different currents the results are harder to analyse. The arcing voltage seems unaltered and non major change can be observed from the 400 A test and the 630 A test regarding the magnitude of the arcing voltage. It might be reasonable to assume that a smaller arcing voltage for higher currents due to a increase in temperature in the arc core. However due to rising temperature energy losses from radiation and other factors increases when interruption in air, as mention in section \ref{sec:HeatTransport}, the relationship between arcing voltage and increased current is not easily illustrated. Since the product of current and electrical conductivity determinates the arcing voltage, and the conductivity is highly dependent on the temperature while the temperature is dependent on the current it is hard to predict the arcing voltage for a certain switchgear design.

The electrical conductivity in the arc is also dependent on its cross-section. When the current increases the cross-section also increases, this might also influence the arcing voltage since the electrical conductivity in the arc maybe better with an increased cross-section, because the current have more space to flow in. However it can be argued that the arc will be cooled more efficient when the surface area of the arc is bigger, as it would be when the cross-section is increased. But generally an increased cross-section results in better conductivity.
...
For the results for geometry \textit{a} these factors seems to compensate for each other and resulting in no significant change in arcing voltage between the two different experiments.
